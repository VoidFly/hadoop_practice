...
public class RelationA implements WritableComparable<RelationA>{
	...
	public boolean isCondition(String attr, char opt, String value){

		int col=0;
		String id_="id";
		String name_="name";
		String age_="age";
		String weight_="weight";
		if(attr.equals(id_))
			col=0;
		else if (attr.equals(name_))
			col=1;
		else if(attr.equals(age_))
			col=2;
		else if(attr.equals(weight_))
			col=3;

		if(opt=='='){
			if(col == 0 && Integer.parseInt(value) == this.id)
				return true;
			else if(col == 1 && this.name.equals(value))
				return true;
		...
		if(opt=='<'){
			if(col == 0 && Integer.parseInt(value) > this.id)
				return true;
			else if(col == 1 && this.name.compareTo(value)<0)
				return true;
			else if(col ==2 && Integer.parseInt(value) > this.age)
				return true;
			else if(col ==3 && Double.parseDouble(value) > this.weight)
